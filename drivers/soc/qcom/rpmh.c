...
>>>>>>> 7457b7d207db (Reapply: Kernel: Xiaomi kernel changes for Redmi K30S Ultra, Mi 10 Ultra, Mi 10 Pro, Redmi K30 Pro and Mi 10 Android R)
 */
	...
	while (i--) {
		if (!oops_in_progress) {
			...
			if (!time_left) {
				 ...
				 */
				rpmh_rsc_debug(ctrlr_to_drv(ctrlr),compls);
				BUG_ON(1);
			}
		} else
			mdelay(100);
